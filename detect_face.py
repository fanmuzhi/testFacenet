...
def getTrainingData(window_name, camera_id, path_name,
                    max_num):  # path_name是图片存储目录，max_num是需要捕捉的图片数量
    ...
    while cap.isOpened():
        ...
        time_str = time.strftime(
                    ...
                    time.localtime(
                        time.time()))
        image_name = os.path.join(path_name, time_str+".jpg")
        num += 1
        cv2.imwrite(image_name, frame)
        # time.sleep(0.5)
        gray = cv2.cvtColor(frame, cv2.COLOR_BGR2GRAY)  # 灰度化
        faceRects = classifier.detectMultiScale(gray, scaleFactor=1.2,
                                                minNeighbors=3,
                                                minSize=(32, 32))
        ...
        if len(faceRects) > 0:
            for faceRect in faceRects:
                ...
                # image = frame[y:y + h,
                #         x:x + w]  # 将当前帧含人脸部分保存为图片，注意这里存的还是彩色图片，前面检测时灰度化是为了降低计算量；这里访问的是从y位开始到y+h-1位
                ...
                cv2.putText(frame, ('%d' % num), (x + 30, y + 30), font, 1,
                            (255, 0, 255),
...
if __name__ == '__main__':
    ...
    getTrainingData('getTrainData', 0, r'./test_image/syna/training/TaiLin',
                    0)
    # getTrainingData('getTrainData', 0, r'./test_image/syna/validate/TaiLin',
    #                 10)